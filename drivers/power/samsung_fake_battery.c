...
#define FAKE_BAT_VOL	4200
#define FAKE_BAT_TEMP	260
...
static struct device_attribute samsung_fake_battery_attrs[] = {
	...
	SAMSUNG_FAKE_BAT_ATTR(batt_lvl),
	SAMSUNG_FAKE_BAT_ATTR(batt_chg),
};
				...
				char *buf)
{
	...
	case BATT_LEVEL:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			samsung_fake_bat_info.bat_info.level);
		break;
	case BATT_CHARGER:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			samsung_fake_bat_info.bat_info.charging_source);
		break;
	default:
		i = -EINVAL;
	}
				...
				const char *buf, size_t count)
{
	...
	case BATT_VOL:
		if (x < 3200 || x > 5000)
			x = FAKE_BAT_VOL;
		samsung_fake_bat_info.bat_info.batt_vol = x;
		dev_info(dev, "%s : batt_vol = %d\n", __func__, x);
		break;
	case BATT_VOL_ADC_CAL:
		...
		break;
	case BATT_TEMP:
		samsung_fake_bat_info.bat_info.batt_temp = x;
		dev_info(dev, "%s : batt_temp = %d\n", __func__, x);
		break;
	case BATT_TEMP_ADC_CAL:
	...
	case BATT_LEVEL:
		if (x < 0 || x > 100)
			x = FAKE_BAT_LEVEL;
		samsung_fake_bat_info.bat_info.level = x;
		dev_info(dev, "%s : batt_level = %d\n", __func__, x);
		break;
	case BATT_CHARGER:
		ret = samsung_cable_status_update(x);
		if (!ret) {
			ret = count;
			dev_info(dev, "%s : batt_charger = %d\n", __func__, x);
		}
		break;
	default:
		ret = -EINVAL;
...
static int __devinit samsung_fake_bat_probe(struct platform_device *pdev)
{
	...
	samsung_fake_bat_info.bat_info.level = 100;
	samsung_fake_bat_info.bat_info.charging_source = CHARGER_AC;
