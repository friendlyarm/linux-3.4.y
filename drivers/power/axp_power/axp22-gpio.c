...
static int __axp_gpio_input(struct gpio_chip *chip, unsigned offset)
{
	...
	else if(GPIO_AXP22(1) == index)
		return axp_gpio_set_io(1, 0);
	else if(GPIO_AXP22(2) == index || GPIO_AXP22(3) == index || GPIO_AXP22(4) == index)
		{
...
//	printk("%s: line %d,%d,\n", __func__, __LINE__,offset);
	...
	if(GPIO_AXP22(0) == index) {
		...
		return value;
	} else if(GPIO_AXP22(2) == index) {
		WARN_ON(0 != axp_gpio_get_value(2, &value));
		return value;
	} else if(GPIO_AXP22(3) == index) {
		WARN_ON(0 != axp_gpio_get_value(3, &value));
		return value;
	} else if(GPIO_AXP22(4) == index) {
