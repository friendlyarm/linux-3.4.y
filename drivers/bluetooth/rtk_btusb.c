				...
				const struct usb_device_id *id)
{
	...
	*/
	...
	data->vcc = regulator_get(NULL, "vgps_3.3V");
	if (IS_ERR(data->vcc)) {
		printk("@@@@@@@@ rfkill: Cannot get regulator for supply %s\n",
			 "vgps_3.3V");
		return -ENODEV;
	}
...
static int btusb_resume(struct usb_interface *intf)
{
	...
	if (test_bit(BTUSB_INTR_RUNNING, &data->flags)) {
	RTKBT_DBG("%s %d BTUSB_INTR_RUNNING",__func__,__LINE__);
	...
	if (test_bit(BTUSB_BULK_RUNNING, &data->flags)) {
	RTKBT_DBG("%s %d BTUSB_BULK_RUNNING",__func__,__LINE__);
	...
	if (test_bit(BTUSB_ISOC_RUNNING, &data->flags)) {
	RTKBT_DBG("%s %d BTUSB_ISOC_RUNNING",__func__,__LINE__);
	...
	void*	unused)
{
	...
	switch (pm_event)
	{
		...
		case PM_POST_RESTORE:
				RTKBT_DBG("rtkbt_pm_notify PM_POST_SUSPEND");
			if (patch_entry->fw_len > 0)
			{
