...
static int nand_block_bad(struct mtd_info *mtd, loff_t ofs, int getchip)
{
	...
	/* SCAN FIRST/LAST - freestyle@20140417 */
	do {
		i = 0;
		...
		ofs = orgofs + (mtd->erasesize - mtd->writesize);
...
read_retry:		/* if fails, read retry same page again */
...
static void nand_set_defaults(struct nand_chip *chip, int busw)
{
	...
	/* TODO: some pointers may need to be reset -- freestyle */
	if (!chip->read_byte)
				...
				u8 id_data[8], int *busw)
{
	...
	printk("  EXT BIT 7:[%x] 6:[%x] 5:[%x] 4:[%x] 3:[%x] 2:[%x] 1:[%x] 0:[%x]\n",
			(extid >> 7) & 1, (extid >> 6) & 1, (extid >> 5) & 1, (extid >> 4) & 1,
			(extid >> 3) & 1, (extid >> 2) & 1, (extid >> 1) & 1, (extid >> 0) & 1);
#endif
	 ...
	 * Toshiba MLC (6 byte ID): Toshiba TC58TEG6DJTA00 (p.61)
	 *
	...
	if (id_len == 6 && id_data[0] == NAND_MFR_SAMSUNG &&
		...
		default: /* Other cases are "reserved" (unknown) */
			if (id_data[1] == 0xD7) /* add for K9GBG08U0B */
				mtd->oobsize = 1024;
			else
				mtd->oobsize = 640;
			break;
						  ...
						  struct nand_flash_dev *type)
{
	...
	*dev_id = chip->read_byte(mtd);
	...
	if (!type->name || !type->pagesize) {
		...
		if (ret) {
			pr_debug (" onfi detected\n");
		}
		else {
			pr_debug (" onfi not detected\n");
		}
		if (ret)
			goto ident_done;
	}
