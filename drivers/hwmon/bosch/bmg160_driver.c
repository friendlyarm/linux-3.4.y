...
#ifdef CONFIG_SENSORS_BMI058
/*Update BMI058 fifo_data_sel to the BMG160 common definition*/
	if (BMI058_FIFO_DAT_SEL_X == fifo_data_sel)
		fifo_data_sel = BMG160_FIFO_DAT_SEL_X;
	if (BMI058_FIFO_DAT_SEL_Y == fifo_data_sel)
		fifo_data_sel = BMG160_FIFO_DAT_SEL_Y;
#endif
	err = sprintf(buf, "%d\n", fifo_data_sel);
	return err;
}
...
/*Update BMI058 fifo_data_sel to the BMG160 common definition*/
		if (BMG160_FIFO_DAT_SEL_X == fifo_data_sel)
			fifo_data_sel = BMI058_FIFO_DAT_SEL_X;
		if (BMG160_FIFO_DAT_SEL_Y == fifo_data_sel)
			fifo_data_sel = BMI058_FIFO_DAT_SEL_Y;
#endif
...
#if 1
	input_set_capability(dev, EV_REL, ABS_MISC);
	dev->relbit[0] = BIT_MASK(REL_RX) | BIT_MASK(REL_RY) | BIT_MASK(REL_RZ);
#else
...
//			BMG_VAL_NAME(MODE_SUSPEND));
	...
	dev_dbg(&client->dev,
	 ...
	 bmg_store_enable(&client->dev, NULL,"1", 0);
